...
import {TreshmodalComponent} from "../treshmodal/treshmodal.component";
...
export class ThreshComponent implements OnInit {
  ...
  createThreshold(dto: ThresholdDto) {
    this.service.createThreshold(dto).subscribe({
      next: (data) => {
        this.toastr.success("Threshold created");
        this.thresholds.push(data);
      }, error: (err) => {
        this.toastr.error(err.error.error);
      }
    })
